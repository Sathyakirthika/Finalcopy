...
const BillingHis = () => {
  ...
  const handleFromDateChange = (date, dateString) => {
    setFromDate(date);
    filterDateData(dateString, toDate);
  };

  const handleToDateChange = (date, dateString) => {
    setToDate(date);
    filterDateData(fromDate, dateString);
  };

  const filterDateData = (from, to) => {
    const filtered = medicineData.filter(
      (item) =>
        ...
        moment(item.createdate)
          .startOf('day')
          .isSameOrAfter(moment(from).startOf('day')) &&
        moment(item.createdate)
          .endOf('day')
          .isSameOrBefore(moment(to).endOf('day'))
    );
    setDateFilteredData(filtered);
  };

  useEffect(() => {
    filterDateData(fromDate, toDate);
  }, []); 
